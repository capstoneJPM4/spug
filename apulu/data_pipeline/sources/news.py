...
class NewsFetcher(DataFetcher):
    ...
    def get_data(self):
        ...
        for c in tqdm(self.news_config['search_keyword']):
            day = start
            while day < end:
                res = _article_search(c, day.strftime('%Y%m%d'), (day + datetime.timedelta(30)).strftime('%Y%m%d'),
                                    self.news_config['base_url'], self.news_config['nyt_key'])
                ...
                for i in res['response']['docs']:
                    tmp = i['web_url']
                    df_article = df_article.append({'Date': day.strftime('%Y-%m-%d'), 'url': tmp}, ignore_index=True)
                if num > 3000:
                    break
            if num > 3000:
                break
